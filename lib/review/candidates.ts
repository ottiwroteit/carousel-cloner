...
function gptPrompt(slot: ReviewSlot): string {
  if (slot.kind === "storefront-hook") {
    return `Photorealistic vertical 9:16 smartphone hero photo for a TikTok grocery carousel titled "${slot.title}". Make it polished and post-ready, with natural light and a clean grocery shopping context.`;
  }
